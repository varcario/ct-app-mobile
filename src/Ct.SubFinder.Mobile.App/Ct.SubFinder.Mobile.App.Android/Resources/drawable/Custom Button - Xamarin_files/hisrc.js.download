...
(function($){
	...
	$.hisrc.defaults = {
		...
		speedTestUri: 'https://xamarin.com/images/50K.jpg',
		speedTestKB: 50,
		speedTestExpireMinutes: 60,
		forcedBandwidth: false,
        srcIsLowResoltion: false, // if true then src will be replaced with 1x or 2x. if false, src will be replaced with 2x
        setWidthHeightOnSwap: false
	};
		...
		var speedTestUri = settings.speedTestUri,
			...
			setImageSource = function ( $el, src ) {
				if ( settings.useTransparentGif ) {
                    ...
                    if (settings.setWidthHeightOnSwap && $el.length > 0) {
                        function load() {
                            if (! $el.prop('complete')) {
                                setTimeout(load, 5);
                            } else {
                                $el.attr('width', $el[0].width);
                                $el.attr('height', $el[0].height);
                                $el.attr('src', src);
                            }
                        }
                        load();
...
// cdeutsch: hard coding init here for Xamarin so you just need to include this file.
$.hisrc.speedTest(); // do speed test ASAP

$(document).ready(function(){
    // selector is backward compatible with retina.js
    $("img[data-retina], img.retina").hisrc({ 
        setWidthHeightOnSwap: false 
    });
});
